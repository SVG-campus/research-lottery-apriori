...
_FIXTURE = Path(__file__).resolve().parent / "fixtures" / "ag_news_stream_sample.json"
...
def test_ag_news_pinned_stream_offline_fixture_matches_charter_schema() -> None:
    """Pinned `datasets.yaml` stream (`fancyzhx/ag_news`): schema + label bounds without Hub I/O."""
...
def test_epistemic_news_charter_perm_budget_respects_smoke_yaml_cap() -> None:
    """`CHARTER_EPISTEMIC_NEWS_STREAM_SMOKE` uses n_perm = max(39, min(149, n_perm_max))."""
